...
/**
 * sqlJDBC provides the database needed for the program to function. It stores ingredients, meals(courses),
 * menus, orders, staff info allergies etc. It will be used as a gateway for different parts of program to
 * interact with each other. For example if waiter confirms an order this order will go to the database,
 * kitchen staff will see new order through the thread, that monitors changes in the database, once an order
 * is confirmed, kitchen will receive a notification about new order.
 * 
 * @author Nikoloz
 *
 */
public class sqlJDBC  {
	...
	public sqlJDBC() throws SQLException, FileNotFoundException {
		...
		//add menuItems to menu.java
		ResultSet menuItemResult = executeSelect(connection, menuItemQuery);
		while( menuItemResult.next() ){
		  Model.getModel().getList().add(MenuItem.toMenuItem(menuItemResult.getString(1)));
		  }
		
		//add ingredient and stock level to stockroom.java
		ResultSet ingredientResult = executeSelect(connection, ingredientQuery);
		while(ingredientResult.next()){
			//TODO after stock room is written change the way we add values to it
			StockRoom.stock.put(Ingredient.toIngredient(ingredientResult.getString(2)), ingredientResult.getInt(1));
		}
