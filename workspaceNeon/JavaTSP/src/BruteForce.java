 ...
 * @version 1.0
 * 
 * Here is where we make the permutations over the array of routes and calculate distances.
 *
 */

public class BruteForce {
   int permutationNumb = 1;
  ...
  public ArrayList<RouteBruteForce> permuteCities(int x, RouteBruteForce currentRoute, RouteBruteForce shortestRoute) {
    ...
    if (x == currentRoute.getCities().size() - 1) {
      if (DriverBruteForce.PRINT_FLAG) {
  ...
  public static int calculateTotalDistance(RouteBruteForce route) {
  ...
  public static String getTotalDistance(RouteBruteForce route) {
