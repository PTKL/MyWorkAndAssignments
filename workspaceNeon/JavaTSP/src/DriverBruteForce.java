...
public class DriverBruteForce {
  final static boolean PRINT_FLAG = false; // can be exchanged with just a false wherever it is tpresent
  ...
  public static void main(String[] args) {
    DriverBruteForce driverBruteForce = new DriverBruteForce();
    Instant startInstant = Instant.now();
    BruteForce bruteForce = new BruteForce();
    RouteBruteForce currentRoute = new RouteBruteForce(initialRoute);
    if (PRINT_FLAG) {
      driverBruteForce.printHeading("RouteBruteForce", " Distance (miles)  |   Shortest Distance  |  Permutation #");
    }
    else {
      System.out.println("permutations in progress ...");
    }
    driverBruteForce.printResult(bruteForce,
        bruteForce.permuteCities(0, currentRoute, new RouteBruteForce(currentRoute)));
    driverBruteForce.printDuration(startInstant);
    
    System.out.println("get totoal distance " + BruteForce.getTotalDistance(currentRoute));
    System.out.println("calc totoal distance " + BruteForce.calculateTotalDistance(currentRoute));
  }
  ...
  public void printResult(BruteForce bruteForce, ArrayList<RouteBruteForce> shortestRoutes) {
    ...
    printHeading("Shortest RouteBruteForce ", "Distance (miles)");
