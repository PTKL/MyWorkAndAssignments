...
public class ChuckDave { 
	...
	System.out.println(extractJoke(requestURL("http://api.icndb.com/jokes/"	+ dayOfYear))); 
	state();
    } 
...
if store[5] != null
	...
	if (json.contains("success")) { 
	    s = json.substring(json.indexOf("\"joke\": ") + 8, json.lastIndexOf(", \"c")); 
	    s = s.replaceAll("&quot;", "\"");
	}
	else 
	...
	if (i == -1) {
	    ...
	    System.out.println(extractJoke(requestURL("http://api.icndb.com/jokes/random"))); 
	else if (i < -1) { 
	    System.out.println("Please enter one of the suggested values"); 
	}
	else 
	    System.out.println(extractJoke(requestURL("http://api.icndb.com/jokes/"+ i)));
	state();
    } 
}
