package tsp.hill.climb;

import java.util.ArrayList;
import org.junit.*;
import static org.junit.Assert.*;

/**
 * The class <code>HillClimbingTest</code> contains tests for the class <code>{@link HillClimbing}</code>.
 *
 * @generatedBy CodePro at 9/20/16 11:21 PM
 * @author emil
 * @version $Revision: 1.0 $
 */
public class HillClimbingTest {
	/**
	 * Run the HillClimbing() constructor test.
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	@Test
	public void testHillClimbing_1()
		throws Exception {
		HillClimbing result = new HillClimbing();
		assertNotNull(result);
		// add additional test code here
	}

	/**
	 * Run the Route findShortestRoute(Route) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	@Test
	public void testFindShortestRoute_1()
		throws Exception {
		HillClimbing fixture = new HillClimbing();
		Route currentRoute = new Route(new ArrayList());

		Route result = fixture.findShortestRoute(currentRoute);

		// add additional test code here
		assertNotNull(result);
	}

	/**
	 * Run the Route findShortestRoute(Route) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	@Test
	public void testFindShortestRoute_2()
		throws Exception {
		HillClimbing fixture = new HillClimbing();
		Route currentRoute = new Route(new ArrayList());

		Route result = fixture.findShortestRoute(currentRoute);

		// add additional test code here
		assertNotNull(result);
	}

	/**
	 * Run the Route findShortestRoute(Route) method test.
	 *
	 * @throws Exception
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	@Test
	public void testFindShortestRoute_3()
		throws Exception {
		HillClimbing fixture = new HillClimbing();
		Route currentRoute = new Route(new ArrayList());

		Route result = fixture.findShortestRoute(currentRoute);

		// add additional test code here
		assertNotNull(result);
	}

	/**
	 * Perform pre-test initialization.
	 *
	 * @throws Exception
	 *         if the initialization fails for some reason
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	@Before
	public void setUp()
		throws Exception {
		// add additional set up code here
	}

	/**
	 * Perform post-test clean-up.
	 *
	 * @throws Exception
	 *         if the clean-up fails for some reason
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	@After
	public void tearDown()
		throws Exception {
		// Add additional tear down code here
	}

	/**
	 * Launch the test.
	 *
	 * @param args the command line arguments
	 *
	 * @generatedBy CodePro at 9/20/16 11:21 PM
	 */
	public static void main(String[] args) {
		new org.junit.runner.JUnitCore().run(HillClimbingTest.class);
	}
}