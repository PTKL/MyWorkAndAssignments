 ...
 * sqrt() function
 * 
 * If the operand is negative then it returns Double.Nan <br>
 * For real numbers it has the same output as Math.sqrt() so a negative number
...
public class SqrtFunction extends Operator {
	...
	public SqrtFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		Argument a0 = args[0];
		if(a0.isNumeric)
			return new Argument(Math.sqrt(a0.toDouble()));
		if(a0.isComplex)
			return new Argument(Complex.sqrt(a0.toComplex()));
