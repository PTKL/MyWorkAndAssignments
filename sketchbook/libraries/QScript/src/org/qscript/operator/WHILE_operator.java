...
/**
 * WHILE(boolean expr) start of the WHILE-END loop. Loop continues if boolean expr = true
 * 
 * @author Peter Lager
 *
 */
public class WHILE_operator extends FlowOperator {

	public WHILE_operator(String symbol, int nbrArgs, int priority, int type) {
		super(FlowOperatorType.WHILE, symbol, nbrArgs, priority, type);
	}
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		Argument a0 = args[0];
		if(a0.isBoolean){
			boolean result = a0.toBoolean();
			return result ? new FlowAction(FlowAction.NEXT) : new FlowAction(FlowAction.JUMP);
		}
