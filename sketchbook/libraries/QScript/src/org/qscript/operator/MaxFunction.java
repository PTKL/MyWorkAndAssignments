...
/**
 * returns larger of two numeric values
 * 
 * @author Peter Lager
 *
 */
public class MaxFunction extends Operator {
	...
	public MaxFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		if(a0.isNumeric && a1.isNumeric){
			if(a0.isInteger && a1.isInteger)
				return  new Argument(Math.max(a0.toInteger(), a1.toInteger()));
			else
				return  new Argument(Math.max(a0.toDouble(), a1.toDouble()));
		}
