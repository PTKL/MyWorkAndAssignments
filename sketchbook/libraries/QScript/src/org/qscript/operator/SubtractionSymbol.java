 ...
 *
 */
public class SubtractionSymbol extends Operator {

	public SubtractionSymbol(String token, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		if(a0.isVector && a1.isVector)
			return new Argument(Vector.sub((Vector)a0.getValue(), (Vector)a1.getValue()));
//		if(a0.isComplex && a1.isComplex)
//			return new Argument(Complex.sub((Complex)a0.getValue(), (Complex)a1.getValue()));
		if(a0.isComplex && a1.isComplex)
			return new Argument( Complex.sub(a0.toComplex(), a1.toComplex()));
		if(a0.isComplex && a1.isNumeric)
			return new Argument( Complex.sub(a0.toComplex(), a1.toDouble()));
		if(a0.isNumeric && a1.isComplex)
			return new Argument( Complex.sub(a0.toDouble(), a1.toComplex()));
