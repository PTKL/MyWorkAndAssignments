...
public final class Geometry2D {
	...
	private static double NEARNESS 			= 1.0;
	 ...
	 */
	public static boolean is_point_near_line(Vector2D v0, Vector2D v1, Vector2D p) {
		Vector2D pnl = point_nearest_line(v0, v1, p);
		if(pnl != null){
			double d = Math.sqrt((p.x-pnl.x)*(p.x-pnl.x) +(p.y-pnl.y)*(p.y-pnl.y)); 
			if (d <= NEARNESS)
				return true;
		}
		return false;
	 ...
	 * @param v1 line end
	 * @param p point to consider
	 * @param vp filled with xy position of the nearest point on finite line (must not be null)
	 * @return true if p is near the line
	 */
	public static boolean is_point_near_line(Vector2D v0, Vector2D v1, Vector2D p, Vector2D vp) {
		Vector2D pnl = point_nearest_line(v0, v1, p);
		if(pnl != null){
			double d = Math.sqrt((p.x-pnl.x)*(p.x-pnl.x) +(p.y-pnl.y)*(p.y-pnl.y)); 
			vp.set(pnl);
	 ...
	 * 
	 * @param v0 line passes through this point
	 * @param v1 line passes through this point
	 * @param p point to consider
	 * @return true if p is near the line
	 */
	public static boolean is_point_near_infinite_line(Vector2D v0, Vector2D v1, Vector2D p) {
	 ...
	 * @param v1 line passes through this point
	 * @param p point to consider
	 * @param vp filled with xy position of the nearest point on line (must not be null)
	 * @return true if p is near the line
	 */
	public static boolean is_point_near_infinite_line(Vector2D v0, Vector2D v1, Vector2D p, Vector2D vp) {
		Vector2D pnl = point_nearest_infinite_line(v0, v1, p);
		if(pnl != null){
			double d = Math.sqrt((p.x-pnl.x)*(p.x-pnl.x) +(p.y-pnl.y)*(p.y-pnl.y)); 
			vp.set(pnl);
	 ...
	 * @param v1 line end
	 * @param p point to consider
	 * @return returns null if the line is undefined or if the nearest point is not on the line
	 */
	public static Vector2D point_nearest_line(Vector2D v0, Vector2D v1, Vector2D p){
		Vector2D vp = null;
		Vector2D the_line = Vector2D.sub(v1, v0);
		double lineMag = the_line.length();
		lineMag = lineMag * lineMag;
		if (lineMag > 0.0) {
			Vector2D pv0_line = Vector2D.sub(p, v0);
	 ...
	 * @param v1 line end
	 * @param p point to consider
	 * @return returns null if the line is undefined else the nearest position
	 */
	public static Vector2D point_nearest_infinite_line(Vector2D v0, Vector2D v1, Vector2D p){
		Vector2D vp = null;
		Vector2D the_line = Vector2D.sub(v1, v0);
		double lineMag = the_line.length();
		lineMag = lineMag * lineMag;
	 ...
	 */
	public static Vector2D line_line_infinite_p(Vector2D v0, Vector2D v1, Vector2D v2, Vector2D v3){
		Vector2D intercept = null;
	 ...
	 * @return an array of coordinates for the intersection if any
	 */
	public static double[] line_line_infinite_p(double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3){
		double[] result = NONE;
		double f1 = (x1 - x0);
		double g1 = (y1 - y0);
		double f2 = (x3 - x2);
